...
module.exports = function (
  ...
  var routes = [
        ...
        function createDevice () {
          ...
          return db.createDevice(account.uid, sessionToken.tokenId, form.device)
            .then(
              function (result) {
                device = result
                log.activityEvent('device.created', request, {
                  uid: account.uid.toString('hex'),
                  device_id: device.id.toString('hex')
                })
              },
              function (err) {
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        function createDevice () {
          ...
          return upsertDevice(request, emailRecord.uid, sessionToken.tokenId, form.device)
            .then(
              function (result) {
                device = result
              },
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        var uid = sessionToken.uid
        var result
        db.deleteDevice(uid, request.payload.id)
          .then(
            function (res) {
              result = res
              return log.activityEvent('device.deleted', request, {
                uid: uid.toString('hex'),
                device_id: request.payload.id
              })
            }
          )
          .then(
        ...
        db.account(Buffer(uid, 'hex'))
          .then(
            function (account) {
              ...
              log.activityEvent('account.verified', request, {
                uid: account.uid.toString('hex')
              })
  ...
  function upsertDevice (request, uid, sessionTokenId, device) {
    ...
    return db[operation](uid, sessionTokenId, device)
      .then(
        function (result) {
          log.activityEvent('device.' + eventSuffix, request, {
            uid: uid.toString('hex'),
